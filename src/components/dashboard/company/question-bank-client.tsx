...
import { Square, CheckSquare, X, Plus, Sparkles } from "lucide-react";
...
export function QuestionBankClient({ questions, user, children }: QuestionBankClientProps) {
  ...
  const columns: ColumnDef<QuestionRow>[] = [
      ...
      render: (row) => {
        ...
        return (
          <div className="flex items-center gap-1">
            ...
            {cats.length > 1 && (
              <span className="inline-flex items-center px-1.5 py-0.5 rounded-full text-[9px] font-extrabold bg-indigo-500/10 text-indigo-500 border border-indigo-500/10">
                +{cats.length - 1}
              </span>
            )}
          </div>
    ...
    },
    {
      ...
      label: "Status",
      render: (row) => (
        <span className="inline-flex items-center px-2 py-0.5 rounded text-[10px] font-bold uppercase tracking-wider bg-indigo-500/10 text-indigo-500 border border-indigo-500/10">
          {row.status || "Active"}
        </span>
      ),
    },
    {
      ...
      label: "Created By",
      sortable: true,
      render: (row) => <span className="text-xs text-muted-foreground truncate max-w-[120px] block">{row.createdByName}</span>,
    },
    {
      key: "createdAt",
      label: "Created At",
  ...
  // If we are on full page forms (add/edit), render them directly
  ...
  return (
    <div className="w-full h-[calc(100vh-168px)] md:h-[calc(100vh-112px)] flex flex-col overflow-hidden -mb-[40px] md:-mb-[48px] relative">
      ...
      {/* Side Slide-Over Panel for details */}
        ...
        {isDetailRoute && (
          <>
            ...
            <motion.div
              ...
              className="fixed inset-y-0 right-0 w-full sm:max-w-lg bg-background/95 backdrop-blur-md border-l border-neutral-200/50 dark:border-neutral-800/80 shadow-2xl z-40 flex flex-col h-full"
            >
